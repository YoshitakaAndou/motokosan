...
class LecDatabaseModel {
  ...
  Future<void> insertLec(Lec _dataDB) async {
  ...
  Future<void> insertLecs(List<Lec> _datesDB) async {
    final Database db = await databaseLec();
    for (Lec _dataDB in _datesDB) {
      await db.insert(
        'lec_table',
        _dataDB.toMap(),
        conflictAlgorithm: ConflictAlgorithm.replace,
  ...
  Future<List<Lec>> getSortedLec() async {
    final Database db = await databaseLec();
    final List<Map<String, dynamic>> maps =
  ...
  Future<List<Lec>> getWhereLecsCategory(String _key) async {
    final Database db = await databaseLec();
    final List<Map<String, dynamic>> maps =
  ...
  Future<List<Lec>> getWhereCategoryAnswered(
  ...
  Future<void> updateLecAtId(Lec _dataDB) async {
  ...
  Future<void> updateLecAtLecId(List<Lec> _datesFB) async {
    final db = await databaseLec();
    for (Lec _dataFB in _datesFB) {
      await db.update(
        'lec_table',
        _dataFB.toMapLecId(),
        where: "lecId = ?",
